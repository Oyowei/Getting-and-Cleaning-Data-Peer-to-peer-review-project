...
if (!file.exists(filename)){
  ...
  download.file(fileURL,destfile = "C:/Users/ESUEZE/Desktop/datasciencecoursera/filename")
}  
...
#1. Merging the data together
X=rbind(x_train, x_test)
Y=rbind(y_train, y_test)
Subject=rbind(subject_train, subject_test)
Data=cbind(Subject, Y, X)

# 2.Extracting the only the means and standard deviation from each measurement
Clean_Data=Data %>% select(subject, code, contains("mean"), contains("std"))
